...
import { sendDeliveryAction, type DeliveryState } from './actions';

/**
 * The "your photos are ready" button on a listing. Which email goes out is
 * decided by the payment lock when it is pressed, and said here before the
 * press, so Nick is never surprised by which version an agent got.
 */
export default function SendDelivery({
  ...
  const blocked = !to
    ? 'Assign an agent to this listing, above, before sending.'
    : mediaCount === 0
      ...
      {blocked ? (
        ...
        <>
          ...
          >
            ...
            <button className="btn btn-primary admin-send" type="submit" disabled={pending}>
              {pending ? 'Sending…' : locked ? 'Send preview email' : 'Send delivery email'}
            </button>
          </form>
